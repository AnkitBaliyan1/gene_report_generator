...
#delete_files_in_folder(glb_01_input)
# Get the file uploaded by the user
file_uploaded = st.file_uploader("Please upload the file here", type='txt')

# This folder path should be defined earlier in your code
...
if file_uploaded is not None:
    ...
    #file_contents = file_uploaded.read().decode('utf-8')

    # Save the contents of the file to a new file on the server
    with open(file_path, 'wb') as file:
        file.write(file_uploaded)
...
with open(os.path.join(glb_01_input,input_file), 'rb') as file:
...
def convert_html_to_pdf2(input_html, output_pdf):
    # Configure pdfkit to use the installed wkhtmltopdf binary path if not in PATH
    #pdfkit_config = pdfkit.configuration(wkhtmltopdf=wkhtml_bin)

    try:
        # Convert html to pdf using pdfkit and wkhtmltopdf
        pdfkit.from_file(input_html, output_pdf
                         # , configuration=pdfkit_config  # Use if wkhtmltopdf is not in PATH
                         )
...
convert_html_to_pdf2(glb_03_output + "/allout.html",glb_03_output + "/output_stage.pdf")
