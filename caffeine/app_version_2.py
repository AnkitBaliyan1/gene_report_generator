...
#glb_root = (r"/home/sangeetha/Downloads/caffeine")
glb_root =("caffeine")
...
ghostscript_bin = "/usr/bin/ghostscript"

#### Step3: import functions from functions_master.py
from functions_linuxmaster_v6 import delete_files_in_folder, read_file_with_skiprows_MyHeritage, read_file_with_skiprows_23andme, read_file_with_skiprows_Ances, read_file_with_auto_detect, get_file_names_in_path, write_dataframes_to_parquet, read_specific_parquet_files, extract_columns_to_dataframe, merge_dataframes_same_colnames, merge_dataframes_differ_colnames,  remove_duplicates_and_create_column, get_icon_number, gen_summary_for_file, save_trait_to_file, get_bg_color, get_all_html_files, convert_html_to_pdf ,remove_before_angle_bracket, get_risk_details
...
## **************************** streamlit user uploading file ***************** ##

import streamlit as st
import os
import time

st.title("Web App for Gene Analysis")
...
file_uploaded = st.file_uploader("Please upload the file here", type='txt')

c_id = st.text_input("enter your Client ID")
...
if st.button("Generate Report"):
    if file_uploaded is not None:
        if c_id== '123':
            ...
            st.error("Enter correct client ID")
    else:
        st.error("upload the file first")


if file_uploaded is not None and access:
    file_upload_flag=True
    #generate_report_flag=True


if file_upload_flag:
    file_path = os.path.join(glb_01_input, file_uploaded.name)

    # Read the contents of the uploaded file
    file_contents = file_uploaded.read()

    # Save the contents of the file to a new file on the server
...
if generate_report_flag and access and file_upload_flag:
    #if st.button("Generate Report"):
        #print("generating report")
    with st.spinner(text="Your report is under process"):
            ...
            input_file = file_names_list[1]
            print("Input file list is: ", file_names_list)
            ...
            for col, bounds in cols_to_transform.items():
                ...
                #outcome = re.search(r"/(.*?)/", col)[0]
                #outcome = re.search(r"/((.*?)/)", col)
                #outcome=$(echo "$col1" | grep -oP '\(.*?\)' | head -n 1)
                #outcome=$(echo "$col" | grep -oE '\([^)]*\)' | head -n 1)
            ...
            #print (glb_03_output + "//allout.html",glb_03_output + "//output_stage.pdf")
            #sys.exit()
            ...
            file_names_list = get_file_names_in_path(glb_03_output)    
            input_file = file_names_list[0]
            print("file_names_list:", file_names_list)
            with open(os.path.join(glb_03_output,input_file), 'r') as file:
                x = file.read()
            
            st.success("Report generated Successfully")
            st.download_button(label="Donwnload Report",
                            data = x,
                            file_name="report",
